...
function as(){$("#dec").val(vd);$("#bin").val(vb);$("#hex").val(vh);}
function dec(num){if(num.match(/^[0-9]+$/)){vd=num; vb=d2b(num); vh=d2h(num); as();} else dec(vd); }
function bin(num){if(num.match(/^[0-1]+$/)){vb=num; vd=b2d(num); vh=num.match(/.{1,32}/g).map(b2h).join(''); as();} else bin(vb); }
function hex(num){if(num.match(/^[A-Fa-f0-9]+$/)){vh=num; vd=h2d(num); vb=h2b(num); as();} else hex(vh); }
